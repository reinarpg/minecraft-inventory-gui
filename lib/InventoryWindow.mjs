...
export class InventoryWindow extends CanvasWindow {
  ...
  render (force) {
    ...
    if (this.needsUpdate || force) {
      ...
      const registerEventForText = (layout, val, handler, xOffset = 0, yOffset = 0) => {
        /** @type {any} */
      ...
      if (this.mobileHelpers) {
        ...
        if (this.xoff > BUTTON_SIZE) {
          ...
          const drawHelper = (x, y, text, isEnabled = true, helperText, callback) => {
            ...
            registerEventForText(layout, text, generatedMethodCallback, -BUTTON_SIZE / 2)
            this.renderLayout(layout, 0, 0)
          }
  ...
  onItemEvent (id, type, pos, data) {
    if (type === 'hover') { // can probably disable tooltips for mobile in some cases
      const [cont, i] = data
